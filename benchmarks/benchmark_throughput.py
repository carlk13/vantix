...
def get_pytorch_loader(paths, size, batch, workers):
    ...
    return DataLoader(VanillaDataset(paths, t), batch_size=batch, num_workers=workers, 
                      pin_memory=True, persistent_workers=True if workers > 0 else False, drop_last=True)
...
def save_time_saved_plots(df):
    ...
    # Plot 1: Throughput
    ax1.bar(x - width/2, df['PT_Speed'], width, label='PyTorch', color='#3498db', alpha=0.8)
    ax1.bar(x + width/2, df['RS_Speed'], width, label='Vantix', color='#2ecc71', alpha=0.8)
...
def rigorous_benchmark(scenario, num_runs=5):
    ...
    if not paths:
        print(f"FATAL ERROR: No images found at path check. Ensure directories exist.")
        sys.exit(1)
    ...
    pt_loader = get_pytorch_loader(paths, scenario['target_res'], scenario['batch'], WORKERS)
    ...
    # Vantix Run
    print(f"    [Vantix] Testing...")
    rs_loader = VantixLoader(image_paths=paths, width=scenario['target_res'], 
                             height=scenario['target_res'], batch_size=scenario['batch'])
...
SCENARIOS = [
    {"name": "Medical-2K", "n_images": 500, "src_res": 2048, "target_res": 224, "batch": 16},
    {"name": "ImageNet-Std", "n_images": 2000, "src_res": 500, "target_res": 224, "batch": 128}
]
...
if __name__ == "__main__":
    ...
    for _, r in df.iterrows():
        ...
        print(f"{r['Scenario']:<15} | Speedup: {r['Speedup']:.2f}x | Time Saved/50k: {time_saved:.1f}s")
